...
var extend=function(des, src) {
 ...
 var Ball=function (diameter,classn) {
    ...
    var ball=document.createElement("div");
		...
		if(ball.id<arr.length){
		ball.innerHTML='<div class="M-bubble"><p style="opacity:1;padding-top:20px;"><strong style="color:#fff;font-size:14px;">'+arr[i]+'</strong>%</p><p>'+arr2[i]+'</p></div>'
		}else{
			ball.innerHTML='<div class="M-bubble"><p style="opacity:1;padding-top:20px;"><strong style="color:#fff;font-size:14px;">'+arr1[i-arr.length]+'</strong></p><p>'+arr2[i]+'</p></div>'
			}
    ...
    with(ball.style) {
		if(i<arr.length){
			...
			width=height=80+'px';position='absolute';
				 ...
				 if(arr1[i-arr.length]<=200){
				width=height=80+'px';position='absolute';
			}
		if(arr1[i-arr.length]<=400&&arr1[i-arr.length]>200){
			width=height=85+'px';position='absolute';
			}	
		if(arr1[i-arr.length]<=600&&arr1[i-arr.length]>400){
			width=height=90+'px';position='absolute';
			}	
		if(arr1[i-arr.length]<=800&&arr1[i-arr.length]>600){
			width=height=100+'px';position='absolute';
			}
		if(arr1[i-arr.length]<=1000&&arr1[i-arr.length]>800){
			width=height=110+'px';position='absolute';
			}	
		if(arr1[i-arr.length]>1000){
			width=113+'px';position='absolute';
			height=95+'px';
			paddingTop=15+'px';
			}			
    }
