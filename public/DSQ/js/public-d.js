...
function loadReportHandle() {
    $.ajax({
        ...
        url: "http://api.tq-service.com/cruise/bigDataInterface/reportHandle",
        success: function(msg) {
            if (msg.status.status_code == 0) {
                var data = msg.data[0].listData;
                var html = "";
                $("#dealPercent").html(msg.data[1].rateData.treatmentRate);
                loadReportCircle(msg.data[1].rateData.treatmentRate)
                for (var i = 0; i < data.length; i++) {
                    html += "<li><div>" + data[i].projectName + "</div>";
                    html += "<div>" + data[i].reportDescribe + "</div>";
                    html += "<div>" + data[i].handlePerson + "</div>";
                    html += "<div>" + data[i].status + "</div>";
                    html += "<div>" + data[i].reportTime + "</div></li>";
                }
                $("#newsConfrom").html(html);
            } else {
                alert("获取数据失败");
            }
        }
...
function loadReportCircle(percent) {
    var p5 = new Progress({
        el: 'my_html4', //canvas元素id
        deg: percent, //绘制角度
        timer: 0.5, //绘制时间
        lineWidth: 5, //线宽
        lineBgColor: '#0A2A5A', //底圆颜色
        lineColor: '#fff', //动态圆颜色
...
function loadNewspaperSource() {
    $.ajax({
        ...
        success: function(msg) {
            if (msg.status.status_code == 0) {
                var data = newspaperSourceData;
                var html = "";
                for (var i = 0; i < data.length; i++) {
                    html += "<div class='M-pre cl'> <div class='cl'>";
                    html += "<h3>" + data[i].name + "</h3>";
                    if (data[i].downOrUpPercent >= 0) {
                        html += "<div class='M-barrate'>环比上升" + data[i].downOrUpPercent + "%<img src='images/d-gr.png' width='10' height='8' alt=''/></div></div>";
                    } else {
                        html += "<div class='M-barrate'>环比下降" + data[i].downOrUpPercent + "%<img src='images/d-red.png' width='10' height='8' alt=''/></div></div>";
                    }
                    html += "<div class='M-prebar'>";
                    html += "<div class='M-bar' style='width:" + data[i].percent + "%;'><div class='progress-value'>" + data[i].percent + "%</div></div>";
                    html += "</div></div>";
                }
                $("#newspaperSource").html(html);
            } else {
                alert("获取数据失败");
            }
        }
    ...
    $.ajax({
        ...
        success: function(msg) {
            if (msg.status.status_code == 0) {
                ...
                htmlCheck += "<td><div><span style='width:0%;'><em></em></span></div></td>";
